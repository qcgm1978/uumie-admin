...
function Ips() {
	...
	this.menu = {};
	this.user = {};
	this.baidu = null, this.geocoder = null, this.google = null, this.g_geocoder = null, this.errormsg = '';
	this.scrollHeight = 0;
	this.options = {
	 ...
	 *            方法名，为空默认当前index。
	 * @param params
	 *            传给服务器数据，和$.ajax.data规则一样。
	 * @param callback
	 *            成功时的回调函数，不设置则使用阻塞模式，否则使用异步
	 * @param onerror
	 *            异常处理回调函数，不设置则使用默认提示
	 * @param remote
	 *            强制决定是否使用远程模式
	 ...
	 * @return 阻塞模式时返回数据
	 */
	this.load = function(module, method, params, callback, onerror, remote, cached, expiration, beforesend) {
		module = module || "index";
		method = method || "index";
		params = params || {};
		...
		$.ajax({
			...
			success: function(json) {
				if (typeof json === "string") {
					...
					try {
						...
						sendMessageToGateway({'HttpError': 500});
						return false;
					}
				} else {
					result = json;
			...
			error: function(XMLHttpRequest, textStatus, errorThrown) {
				try {
					if (XMLHttpRequest.status == 401 && window.top != window) {
						renderLoginToken();
						return;
					} else {
						sendMessageToGateway({'HttpError': XMLHttpRequest.status});
					}
					if (callback)
						callback(null);
		...
		if (code == 403 || code == 510) {
			setTimeout(function() {
				...
				if (!this.errormsg && code == 510)
					this.errormsg = encodeURIComponent(messages[0].substring(10, 19));
				if (this.errormsg)
	...
	this.startup = function(opts) {
		...
		if (result.initjs) {
			...
			this.include(this.appPath + result.initjs + '?timestamp' + (new Date()).getTime(), function() {
		...
		sendMessageToGateway({getTheme : 1});
	};
	...
	this.include = function(urls, callback) {
		if ($.isArray(urls) && jQuery.type(urls[0]) === "string") {
			urls = {load: urls, complete: callback}
	...
	this.initMap = function() {
		$ips.include("http://ditu.google.cn/maps?hl=zh-CN&amp;file=api&amp;v=2&amp;sensor=true&amp;key="
			+ this.options.mapkey);
	};
		...
		for (var i = 0; i < modules.length; i++) {
			...
			switch (module) {
				case MOD_MENU : // 菜单
					...
					files.push(this.appPath + "themes/default/js/common_nav.js");
					break;
				case MOD_HEADER : // 头
				...
				case MOD_MAP : // 地图
					this.google = true;
					// css
					document.write('<link type="text/css" rel="stylesheet" href="' + $ips.appPath
						+ 'css/map_g_set.css" />');
					files.push("http://ditu.google.cn/maps?hl=zh-CN&amp;file=api&amp;v=2&amp;sensor=true&amp;key="
						+ this.options.mapkey);
					break;
				...
				case MOD_BAIDU : // baidu地图
					this.baidu = true;
					document.write('<link type="text/css" rel="stylesheet" href="' + $ips.appPath + 'css/map_g_set.css" />');
//				files.push("http://api.map.baidu.com/api?v=1.3");
					...
					document.write('<script type="text/javascript" src="http://api.map.baidu.com/getscript?v=1.3&key=&services=&t=20121022090043"></script>');
					break;
				case MOD_VD : // 表单验证
					files.push(this.appPath + "js/validate/jquery.validate.js");
				...
				case MOD_DROPDOWN : // 图片下拉
					files.push(this.appPath + "js/msdropdown/jquery.dd.js");
					break;
				case MOD_TIP :
					document.write('<link type="text/css" rel="stylesheet" href="' + $ips.appPath
						+ 'js/poshytip/tip-yellow/tip-yellow.css" />');
					files.push(this.appPath + "js/poshytip/jquery.poshytip.min.js");
					break;
				case MOD_TREE :
					document.write('<link rel="stylesheet" href="' + $ips.appPath
						+ 'js/jquery.ztree/css/zTreeStyle/zTreeStyle.css" type="text/css">');
					files.push(this.appPath + "js/jquery.ztree/js/jquery.ztree.core-3.0.js");
					files.push(this.appPath + "js/jquery.ztree/js/jquery.ztree.excheck-3.0.min.js");
					files.push(this.appPath + "js/jquery.ztree/js/jquery.ztree.exedit-3.0.min.js");
					break;
				case MOD_SEARCH:
					files.push(this.appPath + "themes/default/js/common_fast_search.js");
					break;
				case MOD_MSG:
					files.push(this.appPath + "themes/default/js/common_msg.js");
					break;
			}
		}
		$ips.include(files);
...
Ips.prototype = {
	 ...
	 *            表单ID,不能为空
	 * @param params
	 *            JSON格式,如: {name:'xiaoming',age:20} 也可传String,如:aa=bb&cc=dd
	 *            $param pre 前缀
	 */
	fillFormInput: function(formid, params, pre) {
		...
		for (var i in params) {
			...
			if ($('#' + formid + ' input[name=' + i + ']').length) {
				if ($('#' + formid + ' input[name=' + i + ']').attr("type") == "checkbox") {
					checker = v == 1 ? true : false;
					$('#' + formid + ' input[name=' + i + ']').attr("checked", checker);
				} else {
	...
	/**
	 * 跳转到另一个系统模块的页面
	 *  @param url 目标ur 例如：ucenter/role/index.html
	 *  @param blank     是否在新页面打开，不填 在当前页面打开，true在 新页面打开
	 * 
	 */
	...
	geo : function(q, callback, maptype, centercity) {
		...
		// 经纬度-->地址
		if (latlng.length == 2) {
			var _lat = Math.round((parseFloat(latlng[0]) + 0.0060) * 1000000) / 1000000;
			var _lng = Math.round((parseFloat(latlng[1]) + 0.0065) * 1000000) / 1000000;
			...
			$ips.load('geo','search',{addr:q},function(data) {
				var result = {
					code : 0,
					total: 0,
					poilist: [
				...
				if(data) {
					...
					gc.getPoint(q, function(rs) {
						if (rs) {
							...
							result.poilist = new Array();
							var point = {};
							point.lng = Math.round((parseFloat(rs.lng) - 0.0065) * 1000000) / 1000000;
							point.lat = Math.round((parseFloat(rs.lat) - 0.0060) * 1000000) / 1000000;
							...
							// 				var _pp = poi[i].point;
							// 				var point = {};
							// 				point.lng = Math.round((parseFloat(_pp.lng) - 0.0065) * 1000000)
							// 						/ 1000000;
							// 				point.lat = Math.round((parseFloat(_pp.lat) - 0.0060) * 1000000)
							// 						/ 1000000;
	...
	code2Text : function(module, method, params, mapping, expiration) {
		...
		params = params || {};
		expiration = expiration || 0;
		params = $.param(params) + "&fields=" + mapping.code + "," + mapping.text;
	...
	codeToUpper : function(event, obj) {
		...
		if (key >= 97 && key <= 122) {// 找到输入是小写字母的ascII码的范围
			...
			var _val = $(obj).val();
			$(obj).val(_val.substr(0, pos) + String.fromCharCode(key - 32) + _val.substr(pos, _val.length));// 转换
		}
	},
	...
	// 表格数据加载统一封装函数
	gridLoadData : function(sSource, aoData, fnCallback, module, method, params, preProcess) {
		params = params || [];
		// 参数处理
		var data = {};
		...
		for (var i = 0; i < data["iSortingCols"]; i++) {
			if (i > 0)
				sortColumns += ",";
			sortColumns += fieldDefs[data["iSortCol_" + i]] + " " + data["sSortDir_" + i];
		}
		if (sortColumns != "")
...
if (host == 'ips.9.com') {
	...
	// huoyunren.com
	opts.mapkey = "ABQIAAAAxT6j61Dz1Mr_n1ES5nEL9hSIxEKllCsRa7n6HEZ9fQ-i_5KSoBS_u5pFjsSan3Cvd6QhclSXn8-loA";
